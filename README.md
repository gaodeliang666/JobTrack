...
v0.1.0 项目引导版本已于 2026-07-14 发布。本仓库目前只有项目文档和仓库规则，**尚未创建 Spring Boot 工程骨架，没有可执行入口，暂时不可启动**。文档中提到的业务对象、接口、数据库和统计能力均为后续版本计划，不代表已经实现。
...
本版本未创建 Java 业务代码、Controller、Service、Mapper、`pom.xml`、生产依赖或数据库连接。
